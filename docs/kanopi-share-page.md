# Kanopi private share page

URL (after deploy): https://pedromauri.com/share/kanopi

Default password: `1234`

To change the password, set in Vercel → Project → Settings → Environment Variables:

```
VITE_KANOPI_SHARE_PASSWORD=sua-senha-aqui
```

Then redeploy.

Note: this is a client-side password (fine for sharing application materials with a recruiter; not bank-grade security).

Short email to Vicki:

> Hi Vicki,
>
> I’ve put my written responses (with process images) on a private page:
> https://pedromauri.com/share/kanopi
> Password: 1234
>
> Best,
> Pedro
