...
import {
  ADMIN_PATH,
  ADMIN_STORAGE_KEY,
  PRIVATE_PORTFOLIO_SHARES,
} from "@/content/admin";
import { privatePortfolioSeo } from "@/content/seo";
  ...
  return (
    <section className="mx-auto max-w-2xl px-5 py-16 sm:px-8 sm:py-24">
      ...
      <p className="mt-4 max-w-xl text-lg leading-relaxed text-muted">
        Application shares tailored for specific companies. Each folder keeps its own password when shared
        externally.
      </p>

      <ul className="mt-10 space-y-4">
        {PRIVATE_PORTFOLIO_SHARES.map((share) => (
          <li key={share.to}>
            <Link
              to={share.to}
            ...
            >
              <div className="min-w-0">
                <p className="text-base font-medium text-foreground">{share.label}</p>
                <p className="mt-1 text-sm leading-relaxed text-muted">{share.description}</p>
              </div>
              <ArrowRight
